...
import org.locationtech.jts.jump.workbench.JUMPWorkbench;
...
import org.locationtech.jts.jump.workbench.ui.snap.SnapToFeaturesPolicy;
import org.locationtech.jts.jump.workbench.ui.snap.SnapToGridPolicy;
import org.locationtech.jts.jump.workbench.ui.snap.SnapToVerticesPolicy;
 ...
 * Even if the LayerViewPanel is repainted while the XOR indicator is
 * on-screen.
 */
public abstract class AbstractCursorTool implements CursorTool {
    ...
    public Cursor getCursor() {
        if (cursor == null) {
            cursor = getIcon() instanceof ImageIcon ? GUIUtil
                    .createCursorFromIcon(((ImageIcon) getIcon()).getImage())
    ...
    public static WorkbenchFrame workbenchFrame(LayerViewPanel layerViewPanel) {
        ...
        return (window instanceof WorkbenchFrame) ? (WorkbenchFrame) window
                : null;
    }
    ...
    protected void fireGestureFinished() throws Exception {
        ...
        if (getTaskFrame() != null) {
            ((WorkbenchFrame) SwingUtilities.getAncestorOfClass(
                    WorkbenchFrame.class, getTaskFrame()))
                    .log("Gesture Finished: " + getName());
        }
     ...
     * this method inside #execute(PlugInContext).
     */
