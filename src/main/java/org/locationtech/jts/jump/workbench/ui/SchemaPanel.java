...
import org.locationtech.jts.jump.workbench.ui.addremove.AddRemovePanel;
...
import org.locationtech.jts.jump.workbench.ui.plugin.EditablePlugIn;
...
public class SchemaPanel extends JPanel {
    ...
    private MyPlugIn moveUpPlugIn = new MyPlugIn() {
        ...
        public MultiEnableCheck createEnableCheck() {
            return super.createEnableCheck().add(new EnableCheck() {
                public String check(JComponent component) {
                    return min(rowsToActOn()) == 0 ? "Field is already at the top" : null;
    ...
    void jbInit() throws Exception {
        ...
        forceInvalidConversionsToNullCheckBox.setToolTipText(
            "Leave unchecked if you want to be notified of conversion errors");
        forceInvalidConversionsToNullCheckBox.setText("Force invalid conversions to null");
    ...
    public String validateInput() {
        ...
        if (geometryCount() > 1) {
            return "Only one geometry field is allowed";
        }

        if (geometryCount() == 0) {
            return "A geometry field must be defined";
        }
    ...
    private String validate(int row, String name) {
        ...
        for (int i = 0; i < getModel().getRowCount(); i++) {
            ...
            if (getModel().get(i).getName().equalsIgnoreCase(name.trim())) {
                return "Field name already exists: " + name;
            }
        }
