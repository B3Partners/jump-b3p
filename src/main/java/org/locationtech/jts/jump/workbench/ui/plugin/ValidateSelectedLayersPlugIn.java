...
import org.locationtech.jts.jump.workbench.ui.renderer.style.RingVertexStyle;
...
import org.locationtech.jts.jump.qa.ValidationError;
import org.locationtech.jts.jump.qa.Validator;
...
public class ValidateSelectedLayersPlugIn extends AbstractPlugIn
    ...
    private final static String CHECK_BASIC_TOPOLOGY = "Check basic topology";
    ...
    private final static String CHECK_POLYGONS_HAVE_NO_HOLES = "Disallow Polygons & MultiPolygons with holes";
    private final static String CHECK_NO_REPEATED_CONSECUTIVE_POINTS = "Disallow repeated consecutive points";
    ...
    private void initDialog(PlugInContext context) {
        dialog = new MultiInputDialog(context.getWorkbenchFrame(),
                "Validate Selected Layers", true);
        dialog.setSideBarImage(IconLoader.icon("Validate.gif"));
        dialog.setSideBarDescription("Tests layers against various criteria, " +
            "including polygon orientation, and minimum segment length");
        dialog.addLabel("<HTML><B>GEOMETRY METRICS VALIDATION</B></HTML>");
        dialog.addSeparator();
        ...
        dialog.addCheckBox(CHECK_POLYGON_ORIENTATION,
            false, "Check that polygon shells are oriented clockwise and holes counterclockwise");
        ...
        dialog.addLabel("<HTML><B>GEOMETRY TYPES VALIDATION</B></HTML>");
        dialog.addSeparator();
        ...
        dialog.addCheckBox(DISALLOW_GEOMETRYCOLLECTIONS,
            false, "Geometry-collection subtypes (like multi-points) are not disallowed");
